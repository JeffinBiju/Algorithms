...
void dfs(ll u,ll p,vector<vector<pair<ll,ll> > >&adj,vector<ll> &par,ll &tim,vector<ll> &tin,vector<ll> &tout,ll d,vector<ll> &dist){
    par[u]=p;
    tin[u]=tim;
    tim++;
    dist[u]=d;
    for(auto x:adj[u]){
        ...
        ll e=x.second;
        if(u2!=p)
        dfs(u2,u,adj,par,tim,tin,tout,d+e,dist);
    }
    tout[u]=tim;
    tim++;
    return;
