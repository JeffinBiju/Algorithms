...
void dfs(ll u,ll p,vector<vector<ll> >&adj,vector<bool> &vis,vector<bool> &ap,vector<ll> &tim,vector<ll>&low,ll &cur){
    //cout<<u<<endl;
    vis[u]=1;
    tim[u]=cur;
    cur++;
    ...
    for(auto x:adj[u]){
        if(vis[x]){
            ...
            cnt++;
            dfs(x,u,adj,vis,ap,tim,low,cur);
...
void solve(ll n,ll m){
     ...
     for(ll i=1;i<=n;i++){
         if(!vis[i]){
             dfs(i,i,adj,vis,ap,tim,low,cur);
         }
     }
     ll cnt=0;
     for(ll i=1;i<=n;i++){
